...
def plot_num(switzerland,mode=0,title='Carte de la Suisse'):
    ...
    custom_cmap = LinearSegmentedColormap.from_list('CustomRedToGreen', cmap_colors, N=256)
    ...
    if mode == 0:
        switzerland.plot(column='DATA', cmap=custom_cmap,ax=ax, legend=True,norm=norm)
    if mode == 1:
        switzerland.plot(column='DATA', cmap="RdBu_r", ax=ax, legend=True,norm=norm)
...
def plot_nb_elu_party(dataset,party,year_old,year_new):
    ...
    fig=plot_num(switzerland_data,0,'Party')
    return fig
...
def plot_diff_gender(dataset,gender,year_old,year_new):
    ...
    switzerland = creat_geoswitz('Dataset/Swiss.geojson')
    switzerland_data = fusion_data_map(switzerland, subset_com)
    fig=plot_num(switzerland_data,0,'Genre')
    return fig
def plot_best_party(dataset,year):
    ...
    fig=plot_obj(switzerland_data,"Partis")
    return fig
